...
R3BDeadLDigitizer::R3BDeadLDigitizer()
    : R3BDeadLDigitizer("R3BDeadLDigitizer", 1)
{
}

// R3BAlpideDigitizer: Standard Constructor --------------------------
R3BDeadLDigitizer::R3BDeadLDigitizer(const TString& name, Int_t iVerbose)
...
void R3BDeadLDigitizer::SetParContainers()
{
...
void R3BDeadLDigitizer::SetParameter()
{
    //--- Parameter Container ---
    if (fMappingPar)
    {
...
// ----   Public method Init  -----------------------------------------
InitStatus R3BDeadLDigitizer::Init()
{
    ...
    fAlpideHits = new TClonesArray("R3BDeadLHitData");
...
void R3BDeadLDigitizer::Exec(Option_t*)
{
    Reset();
    ...
    R3BDeadLPoint** pointData = new R3BDeadLPoint*[nHits];
    ...
    {
        fRot.SetToIdentity();
        pointData[i] = dynamic_cast<R3BDeadLPoint*>(fAlpidePoints->At(i));
        ...
        auto fClusterSize = static_cast<uint16_t>(eLoss * pixelSizeFactor2 + pixelSizeOffset2);
...
InitStatus R3BDeadLDigitizer::ReInit()
{
    SetParContainers();
    SetParameter();
    return kSUCCESS;
...
R3BDeadLHitData* R3BDeadLDigitizer::AddHitData(UInt_t sid, uint16_t clustersize, Double_t x, Double_t y, Double_t z)
{
    // It fills the R3BAlpideHitData
    TClonesArray& clref = *fAlpideHits;
    Int_t size = clref.GetEntriesFast();
    return new (clref[size]) R3BDeadLHitData(sid, clustersize, x, y, z);
}

ClassImp(R3BDeadLDigitizer)
