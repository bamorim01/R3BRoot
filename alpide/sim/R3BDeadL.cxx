...
R3BDeadL::R3BDeadL(const TString& geoFile, const TGeoTranslation& trans, const TGeoRotation& rot)
    : R3BDeadL(geoFile, { trans, rot })
{
}

R3BDeadL::R3BDeadL(const TString& geoFile, const TGeoCombiTrans& combi)
    : R3BDetector("R3BDeadL", kTRA, geoFile, combi)
    , fAlpidePoint(new TClonesArray("R3BDeadLPoint"))
    , fMap_Par(nullptr)
    , fAlpideGeo(nullptr)
    , fGeoversion(2024)
{
...
void R3BDeadL::SetSpecialPhysicsCuts()
{
...
Bool_t R3BDeadL::ProcessHits(FairVolume*)
{
...
void R3BDeadL::Register() { FairRootManager::Instance()->Register("DeadLPoint", GetName(), fAlpidePoint, kTRUE); }
// ----------------------------------------------------------------------------

// -----   Public method GetCollection   --------------------------------------
TClonesArray* R3BDeadL::GetCollection(Int_t iColl) const
{
    if (iColl == 0)
    {
        return fAlpidePoint;
...
// -----   Public method Print   ----------------------------------------------
void R3BDeadL::Print(Option_t*) const
{
...
R3BDeadLPoint* R3BDeadL::AddHit(Int_t trackID,
                                  Int_t detID,
                                  Int_t detCopyID,
                                  TVector3 posIn,
                                  TVector3 posOut,
                                  ...
                                  Int_t pdgcode)
{
    ...
    return new (clref[size])
        R3BDeadLPoint(trackID, detID, detCopyID, posIn, posOut, momIn, momOut, time, length, eLoss, pdgcode);
}

// -----   Public method CheckIfSensitive   -----------------------------------
Bool_t R3BDeadL::CheckIfSensitive(std::string name)
{
