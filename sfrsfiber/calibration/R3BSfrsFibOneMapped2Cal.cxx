...
#include "R3BSfrsFibOneMapped2Cal.h"
...
R3BSfrsFibOneMapped2Cal::R3BSfrsFibOneMapped2Cal()
    : R3BSfrsFibOneMapped2Cal("R3BSfrsFibOneMapped2Cal", 1)
{
}

R3BSfrsFibOneMapped2Cal::R3BSfrsFibOneMapped2Cal(const TString& name, Int_t iVerbose)
...
InitStatus R3BSfrsFibOneMapped2Cal::Init()
{
    ...
    fMappedData = dynamic_cast<TClonesArray*>(mgr->GetObject("SfrsFibOneMappedData"));
    if (!fMappedData)
    {
        R3BLOG(fatal, "SfrsFibOneMappedData not found");
        return kFATAL;
    }

    fCalData = new TClonesArray("R3BSfrsFibOneCalData");
    mgr->Register("SfrsFibOneCalData", "SfrsFibOne_Cal", fCalData, !fOnline);
    Reset();

    return kSUCCESS;
}

void R3BSfrsFibOneMapped2Cal::Exec(Option_t*)
{
    Reset();
    ...
    {
        auto const* mapped = dynamic_cast<R3BSfrsFibOneMappedData const*>(obj);
        if (!mapped) continue;
...
void R3BSfrsFibOneMapped2Cal::Reset()
{
    if (fCalData) fCalData->Clear();
}

double R3BSfrsFibOneMapped2Cal::BuildTimeNs(uint16_t side, uint16_t type, uint16_t edge, uint16_t channel, uint16_t coarse, uint16_t fine) const
{
    return static_cast<double>(coarse) * fCoarseClockNs - GetFineTimeNs(side, type, edge, channel, fine);
}

double R3BSfrsFibOneMapped2Cal::GetFineTimeNs(uint16_t, uint16_t type, uint16_t, uint16_t, uint16_t fine) const
{
...
R3BSfrsFibOneCalData* R3BSfrsFibOneMapped2Cal::AddCalData(uint16_t side, uint16_t type, uint16_t channel, double leadingTimeNs, double trailingTimeNs, double totNs)
{
    TClonesArray& clref = *fCalData;
    const auto size = clref.GetEntriesFast();
    return new (clref[size]) R3BSfrsFibOneCalData(side, type, channel, leadingTimeNs, trailingTimeNs, totNs);
}

ClassImp(R3BSfrsFibOneMapped2Cal)
